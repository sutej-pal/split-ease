-- Fix: generic group share-link invites were auto-accepted by the inviter.
-- createFreshGroupShareLink stored the inviter's email on the invite row, so
-- accept_pending_invites() matched that email on the inviter's next sync and
-- marked the link ACCEPTED before any invitee opened it.
--
-- Apply in Supabase SQL Editor (existing projects). Fresh DBs get the same
-- logic via docs/sql/migration_db.sql.

-- 1) Email-based accept: only person-specific invites (friend_row_id set).
--    Share links (friend_row_id null) are claimable only via accept_invite_by_token.
...
-- 2) Token accept: refuse inviter self-claim; keep share links PENDING (multi-use).
...
-- 3) Heal burned share links (inviter email auto-accept) and normalize placeholder email.
update public.invites
set status = 'PENDING',
...
-- Preview member list: omit generic share-link rows (placeholder email looks like a guest).
