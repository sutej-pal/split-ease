...
- **Flush then pull:** `SyncInteractor.syncForUser` flushes PENDING groups/members/invites/expenses/payments/activity events/pin boards, then pulls friends/groups/invites in parallel, then expenses + payments + activity in parallel (payments use batched `group_id in (...)` like expenses). Also runs on login / cold start / group resume (no manual Account Sync action). Groups home paints the Room group list before the first overall-balance pass finishes and skips friend×group pairwise nets on that screen.
