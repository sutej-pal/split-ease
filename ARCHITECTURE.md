...
- **Flush then pull:** `SyncInteractor.syncForUser` flushes PENDING groups/members/invites/expenses/payments/activity events/pin boards, then pulls friends/groups/invites in parallel, then expenses + payments + activity in parallel (payments use batched `group_id in (...)` like expenses; `fetchCompleteInFilter` pages past PostgREST’s 1000-row cap, and group prune treats that result as complete). Also runs on login / cold start / group resume (no manual Account Sync action). Groups home paints the Room group list before the first overall-balance pass finishes, skips friend×group pairwise nets on that screen, and keeps the last overall-balance snapshot while pull-to-refresh freezes amounts.
...
- **Schema SoT:** [docs/data-dictionary.md](docs/data-dictionary.md) + `app/schemas/` (Room **v17**). Incremental Room migrations are not used while the app is in development; `DatabaseModule` uses `fallbackToDestructiveMigration` so schema bumps wipe the local cache and the next sync rehydrates from Supabase. Reintroduce `Migration` objects before the first production Play ship.
...
| What's new               | `ChangelogParser`, `presentation/changelog`; Play copy from `./gradlew newRelease` → `play/whatsnew/en-US.txt`                                                                             |           |          |
