...
- **SplitEase Server (separate repo)** — lives at `C:\splitease\server` beside the Android app at `C:\splitease\app` ([docs/splitease-server-repo.md](docs/splitease-server-repo.md)). Known-good: `b04a42d`. Prefer Nodemailer SMTP locally. Render Free blocks SMTP — use local/ngrok, paid host, or Resend + verified domain.
- **Render Free SMTP** — outbound SMTP ports are blocked on Free; onboarding mail needs Resend HTTPS + verified domain on Render, or a paid instance / local SMTP for Gmail ([phase-10d](docs/phase-10d-onboarding-mail.md)).
...
- **Signup phone duplicate RPC** — Apply [sql/phase-auth-phone-registered.sql](docs/sql/phase-auth-phone-registered.sql) on existing Supabase projects if not yet run (`auth_phone_registered`).
