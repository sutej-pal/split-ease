...
- **App Links / invite https** — share links use `MAIL_SERVICE_BASE_URL/invite/{token}`; mail-service tries open-app then Play Store with `invite_token` Install Referrer. Full App Links (`assetlinks.json`) still TODO ([phase-10c](docs/phase-10c-invite-join.md)). Redeploy mail-service after pulling invite-page changes.
