...
class MissingFxColumnTest {
    @Test
    fun detects_postgrest_unknown_fx_column() {
        ...
        assertFalse(isMissingFxColumn(RuntimeException("PGRST204 Could not find the 'is_recurring' column")))
        assertFalse(isMissingFxColumn(RuntimeException("check constraint original_amount violated")))
    }
